...
export type Props = StyleProps & { toggleState?: "on" | "off", icons?: { on: JSX.Element, off: JSX.Element } }
...
export const ToggleInput = makeComponent({
	defaultProps: () => Object.freeze({
		...
		icons: {
			...
			off: <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
	...
	const { style, icons, postMsgAsync, children } = props
	...
	return <div
		...
		onClick={e => {
			...
			setState({ toggleState: newToggleState })
			postMsgAsync({ type: "TOGGLE", data: { toggleState: newToggleState } })
		}}>

		{toggleState === "on" ? icons.on : icons.off}
	</div>
})